 ...
 * LevelGift entities. Transaction control of the save(), update() and delete()
 * operations can directly support Spring container-managed transactions or they
 * can be augmented to handle user-managed Spring transactions. Each of these
 * methods provides additional information for how to configure it for the
 * desired type of transaction control.
 * 
 * @see com.LevelGiftDTO.hibernate.LevelGift
 * @author MyEclipse Persistence Tools
 */
public class LevelGiftDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory
			.getLogger(LevelGiftDAO.class);
	// property constants
	...
	public LevelGiftDTO findById(java.lang.Integer id) {
		...
		try {
			LevelGiftDTO instance = (LevelGiftDTO) getSession().get(
					"com.test.hibernate.LevelGift", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public LevelGiftDTO merge(LevelGiftDTO detachedInstance) {
		...
		try {
			LevelGiftDTO result = (LevelGiftDTO) getSession().merge(detachedInstance);
